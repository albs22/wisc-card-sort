...
Ext.ux.TouchGridPanel = Ext.extend(Ext.Panel, {
	...
	buildHeader   : function() {
		...
		colTpl += '    <tr>';
		//sets column values from colModel
	...
	buildDataView : function() {
		...
		//Style first column header
			...
			colTpl += '<td width="' + width + '%" class="x-grid-cell1 x-grid-col-   x-grid-header' + col.mapping + ' ' + cls + ' {isDirty:this.isCellDirty(parent)}" style="' + style + '" mapping="' + col.mapping + '" rowIndex="{rowIndex}">{' + col.mapping + '}</td>';
